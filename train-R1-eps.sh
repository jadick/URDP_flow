#!/bin/bash

sbatch <<EOF
#!/bin/bash
#SBATCH --gres=gpu:t4:1
#SBATCH -c 10
#SBATCH --mem=40GB
#SBATCH --time=12:00:00
#SBATCH --job-name=new-$2-25
#SBATCH --output=exp.log
#SBATCH --qos=m
#SBATCH --exclude=gpu096,gpu106,gpu071,gpu069,gpu062
# activate environment
module load pytorch2.0-cuda11.8-python3.9
#pytorch1.7.1-cuda11.0-python3.6 
#pytorch1.13-cuda11.7-python3.7


python train-R1eps-FAST.py --total_epochs $3 \
				--lambda_JD 0 \
				--lambda_MSE 1 \
				--lambda_NEW 25 \
				--lambda_FMD 0 \
				--lambda_gp 10 \
				--bs 64 \
				--skip_fq 5 \
				--dim 128 \
				--z_dim $1 \
				--L 2 \
				--d_penalty 0 \
				--pre_path './saved_models/$2/R1-eps|dim_128|z_dim_$1|L_2|lambdaJD_0.0|lambdaFMD_0.0|lambdaNEW_0.025|lambdaMSE_1.0'\
				--path './data/' 

#'./saved_models/12/R1-eps|dim_128|z_dim_6|L_2|lambdaJD_0.0|lambdaFMD_0.0|lambdaNEW_0.0|lambdaMSE_1.0'\

EOF
